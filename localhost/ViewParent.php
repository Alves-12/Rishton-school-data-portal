...
<body>
  ...
  <table>
    ...
    <tr>
      ...
      <br>
      <th width="250px">Parent Name<br>
        <hr>
      </th>
      <br>
      <th width="250px">Parent DOB<br>
        <hr>
      </th>
      <br>
      <th width="250px">Parent Address<br>
        <hr>
      </th>
      <br>
      <th width="250px">Parent_Email<br>
        <hr>
      </th>
      <br>
      <th width="250px">Parent_PhoneNumber<br>
        <hr>
      </th>
      <br>
      <th width="250px">PupilID<br>
        <hr>
      </th>
    </tr>

    <?php
    $sql = mysqli_query($link, "SELECT ParentID, Parent_Name, Parent_DOB, ParentAddress, Parent_Email, Parent_PhoneNumber, PupilID FROM Parent");
    while ($row = $sql->fetch_assoc()) {
      echo "
	<tr>
		<th>{$row['ParentID']}</th>
		<th>{$row['Parent_Name']}</th>
    <th>{$row['Parent_DOB']}</th>
		<th>{$row['ParentAddress']}</th>
		<th>{$row['Parent_Email']}</th>
		<th>{$row['Parent_PhoneNumber']}</th>
		<th>{$row['PupilID']}</th>
	</tr>";
    }
    ?>
